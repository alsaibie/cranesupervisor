...
ssize_t UART::rx_dma_buffer(const uint8_t *buf, size_t len) {
  ...
  while (len--) {
    ...
    if (idx > 0 && (c < 0 || c == LF || c == CR || c == CRLF)) { /* Accept any line termination */
...
//TODO: Implement as virtual
//void UART::process_line(const uint8_t *buf, size_t len) {
//
//#if ECHO_TEST
...
size_t UART::print(const char *buf) {
  ...
  return tx_dma_buffer((uint8_t*) (buf - len - 1), len);
...
void UART::clear_rx_dma_buffer(){
  memset(dma_rx_buffer, 0, UART_RX_BUFFER_SIZE);
}
